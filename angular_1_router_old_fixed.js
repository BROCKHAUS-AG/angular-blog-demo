    ...
    function outletLink(scope, element, attrs, ctrls, $transclude) {
        var Outlet = (function () {
            ...
            Outlet.prototype.activate = function (instruction) {
                ...
                var nextScope = scope.$new();
                var nextElement = $transclude(nextScope, function (clone) {});
                // TODO: prefer the other directive retrieving the controller
                // by debug mode
                var nextController = nextElement.children().eq(0).controller(componentName);

                var onActivate = $q.when();
                if (nextController && nextController.$routerOnActivate) {
                    onActivate = $q.when(nextController.$routerOnActivate(instruction, previousInstruction));
                }
                return onActivate.then(function () {
                    $animate.enter(nextElement, null, _this.currentElement || element);
                    _this.cleanupLastView();

                    _this.currentElement = nextElement;
                    _this.currentScope = nextScope;
                    _this.currentController = nextController;
                }, function () {
                    nextScope.$destroy();
                });
            };
